...
class Program
{
    ...
    static void Main()
    {
        var GetFiltered = delegate (int[] arr, Func<int, bool> filter)
        { return arr.Where(item => filter(item)).ToArray(); };
        var Print = delegate (int[] arr)
        {
        ...
        int[] has3Array = GetFiltered(array, delegate (int num)
        {
            do
            {
                if (num % 10 == 3)
